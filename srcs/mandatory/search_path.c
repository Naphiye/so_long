...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/02 17:56:24 by fsalomon          #+#    #+#             */
/*   Updated: 2024/04/03 10:10:44 by fsalomon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

static int	ft_can_go(t_Direction d, t_position current_pos, char **map)
{
...
static int	ft_which_directions(t_map *new_map, t_position current_pos,
		t_position *path, int path_len)
{
	...
	up = UP;
	down = DOWN;
	if (current_pos.x == 1 || !ft_can_go(LEFT, current_pos, new_map->map)
		|| ft_on_path(LEFT, current_pos, path, path_len))
		left = 0;
	if (current_pos.x == (*new_map).width - 2 || !ft_can_go(RIGHT, current_pos,
			new_map->map) || ft_on_path(RIGHT, current_pos, path, path_len))
		right = 0;
	if (current_pos.y == 1 || !ft_can_go(UP, current_pos, new_map->map)
		|| ft_on_path(UP, current_pos, path, path_len))
		up = 0;
	if (current_pos.y == (*new_map).height - 2 || !ft_can_go(DOWN, current_pos,
			new_map->map) || ft_on_path(DOWN, current_pos, path, path_len))
		down = 0;
	return (left + right + up + down);
}

static int	ft_explore_all_path(t_position *path, t_position *open_nodes,
		t_map *new_map, t_position *current_pos)
{
	...
	while (1)
	{
		dir_possible = ft_which_directions(new_map, *current_pos, path,
				path_len);
...
static int	ft_search_path(t_map *new_map, t_position *current_pos)
{
	t_position	*path;
	...
	is_there_a_valid_path = ft_explore_all_path(path, open_nodes, new_map,
			current_pos);
...
int	ft_check_path(t_map *new_map)
{
	...
	return (ft_search_path(new_map, &current_pos));
}
