...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/02 15:31:46 by fsalomon          #+#    #+#             */
/*   Updated: 2024/04/03 11:06:16 by fsalomon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_init_all_img_bonus(t_game *game_data)
{
	...
	game_data->wall.img_wall3 = ft_init_img(game_data, "./TEXTURES/wall3.xpm");
	game_data->wall.img_wall4 = ft_init_img(game_data, "./TEXTURES/wall4.xpm");
	game_data->mob.img = ft_init_img(game_data, "./TEXTURES/mob.xpm");
	...
	if (!(game_data->exit.img_close && game_data->exit.img_open
			...
			&& game_data->wall.img_wall3 && game_data->wall.img_wall4
			&& game_data->mob.img))
		close_window(game_data);
	return ;
}

void	ft_display_map_bonus(t_map *new_map, t_game *game_data, int x, int y)
{
	while (y < new_map->height)
	{
		...
		while (x < new_map->width)
		{
			...
			else if (new_map->map[y][x] == 'M')
				mlx_put_image_to_window(game_data->mlx_ptr, game_data->mlx_win,
					game_data->mob.img->img, x * 48, y * 48);
			x++;
		}
		y++;
	}
...
int	ft_parse_map_bonus(char *map_str, t_map *new_map)
{
	...
	error = 0;
	if (!ft_check_symbol_bonus(map_str))
		error += 1;
