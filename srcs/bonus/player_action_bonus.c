...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/02 15:26:08 by fsalomon          #+#    #+#             */
/*   Updated: 2024/04/03 10:48:29 by fsalomon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_end_game_bonus(int x, int y, char *path, t_game *game)
{
	...
	mlx_put_image_to_window(game->mlx_ptr, game->mlx_win, game->img_end->img, 0,
		0);
	mlx_hook(game->mlx_win, 2, 1L << 0, key_hook_bonus, game);
	mlx_hook(game->mlx_win, 17, 1L << 17, close_window, game);
}

void	ft_move_player_at_bonus(int x, int y, t_game *game, t_img *img_dir)
{
